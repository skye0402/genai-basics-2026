...
export function registerTimeAndPlaceTool(server: McpServer): void {
  registerToolWithLogging(
    ...
    'get_time_and_place',
    {
      ...
      description:
        'Returns the current local date/time and an inferred place (capital city for the system\'s country when possible). No input parameters.',
      // No input schema => tool takes no parameters
      outputSchema: timeAndPlaceResultSchemaShape,
    },
    async () => {
      const output = inferTimeAndPlace();
