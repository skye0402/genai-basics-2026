...
async def _process_untitled_sessions():
    """
    ...
    try:
        ...
        for session_item in sessions:
            ...
            session = storage.get_session(session_item.session_id)
        ...
        if untitled_count > 0:
            logger.info(f"Processed {untitled_count} untitled sessions")
        else:
            logger.debug("No untitled sessions to process")
...
async def create_chat_session(
    ...
    """
    storage = get_storage()
    session = storage.create_session(title=request.title)
    
    # Process old sessions without titles in the background
    background_tasks.add_task(_process_untitled_sessions)
...
async def delete_chat_session(session_id: str):
    ...
    if len(remaining_sessions) == 0:
        ...
        logger.info("Last session deleted, creating new default session")
        new_session = storage.create_session(title="New Chat")
...
async def _generate_title_background(session_id: str):
...
async def append_chat_message(
    ...
    if not updated_session.title_generated and len(updated_session.messages) >= 4:
        ...
        background_tasks.add_task(_generate_title_background, session_id)
